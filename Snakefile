...
rule classify_variants:
    ...
    params:
        cutoff = lambda wc: config["genes"][wc.gene].get("exon_cutoff"),
    log:
    ...
    shell:
        ...
        mkdir -p results
        CUTOFF_ARG=""
        if [ "{params.cutoff}" != "None" ] && [ -n "{params.cutoff}" ]; then
            CUTOFF_ARG="--exon-cutoff {params.cutoff}"
        fi
        python scripts/classify.py \
            --input {input} --output {output} \
            --gene {wildcards.gene} $CUTOFF_ARG > {log} 2>&1
        """
