...
# Fail loudly if more than this fraction of gnomAD variants have no
# matching ANNOVAR row - almost always a real bug (build mismatch, indel
# position convention) rather than an expected gap, and would otherwise
# silently degrade classification (blank predictor columns just fail the
# missense filter instead of erroring).
MAX_MISS_FRACTION = 0.01
...
def main():
    ...
    miss_fraction = (n_missed / len(grows)) if grows else 0
    if miss_fraction > MAX_MISS_FRACTION:
        sys.exit(
            f"ERROR: {n_missed}/{len(grows)} variants ({miss_fraction:.1%}) "
            f"had no matching ANNOVAR annotation, exceeding the "
            f"{MAX_MISS_FRACTION:.0%} threshold - check build/version "
            f"mismatch or indel position conventions between the gnomAD "
            f"CSV and the ANNOVAR output."
        )
